...
# 240709 #
**개인공부** : 
- 자바스크립트 복습 : 함수, 화살표 함수, 객체

# 240708 #
**개인공부** : 
- 자바스크립트 복습 : 변수, if 조건문, 반복문(while, for), switch문

# 240702 #
**수료식** : 
- 포트폴리오 제작과정 발표 준비 및 발표
- 수료
...
# 240626 #
**반응형 개인 프로젝트** : 
- 스크롤 이벤트 추가, GSAP 이벤트 추가
- introduction 컨텐츠 css 수정
- guide 컨텐츠 css 수정
- 태블릿 및 모바일 버전 css 작업
- 포트폴리오 상세 페이지 작성
...
- 개인 프로젝트 사이트 디자인
- 해야할 일 
1. 페이지 및 아이템 정렬 (한 눈에 들어오게)
2. 아이콘 변경 (좀 더 눈에 띄는 걸로)
3. 사진에 주는 포인트 변경(포인트가 눈에 잘 안들어옴)
...
- swiper autoplay 설정
- 빌드 후 배포
- 배포 환경에서 빈화면이 나오는 이슈 발생 및 라우터 구조 수정 후 해당 컴포넌트로 이동 안되는 이슈 발생
...
- 주차장 예약 시스템
- form 제어
- encodeURIComponent(); URI의 일부로 사용될 특수문자 포함된 데이터의 문자열을 인코딩하여 전달
- URI: 인터넷상의 모든 자원 혹은 데이터 / 통합자원식별자
- useLocation();
- useEffect : 컴포넌트 생명주기마다 특정 기능을 실행 할 수 있게 해주는 hook
...
- 변수 대입 방법 -> 자바스크립트로 작성, 모두 {} 안에 변수명 작성
- 리액트에서의 .map() 작성 특징
...
- .data();
- gsap을 사용하여 https://qude.audio/ 사이트 따라하기
- 데이터가 불러오기 전까지 로딩되는 페이지 제작

**자바스크립트 팀 프로젝트** :
- 메인 페이지 테블릿 버전 css 작업
- 마우스 허버 이벤트, 서브 페이지 디자인 통일
- 제이쿼리 이벤트 추가
...
# 240516 #
**jQuery** : 
- 스크롤 시 가로로 이미지가 넘어가는 이벤트
- resize : 리사이즈 이벤트가 발생했을 때 (반응형 웹 사이트)
- vegas- pulgIn 사용 방법
- swiper pulgIn 사용 방법

**자바스크립트 팀 프로젝트** :
- 피시 버전 메인 페이지 css 작업
- 제이쿼리/ 자바스크립트 이벤트 작업, 애니메이션 작업

# 240515 #
**jQuery** : 
- $.each(배열 혹은 객체, 콜백함수);
- requestAnimationFrame(); -> 화면에 프레임이 실행되기 전 함수 호출
- 마우스 좌표(브라우저 기준, 요소기준, 페이지기준, 디바이스기준)

**자바스크립트 팀 프로젝트** :
- 모바일 버전 메인 페이지 css 작업
...
- 제이쿼리 UI 색상, 움직임 , 스타일 등을 위해 파일 연결 /jquery-ui.min.js
- e.keycode/ 키보드로 이벤트 제어
- animate 예제들
- 자바스크립트 팀 프로젝트
...
**jQuery** : 
- 자바스크립트 문제 풀이
- transition: 1s 0.8s; /* 진행시간, 대기시간 */
- hasClass(); 기준 요소에 해당 클래스가 있으면 true, 없으면 false 반환
- remove(); 기준 요소부터 하위 노드 전부 제거. 기준 요소도 제거
- empty(); 기준 요소의 하위 노드만 제거. 기준 요소는 남음
- $('B').prependTo('A'); 부모인 A의 자식들 중 요소 B를 자식들 중 제일 처음으로 '이동'
- $('B').appendTo('A'); 부모인 A의 자식들 중 요소 B를 자식들 중 제일 마지막으로 '이동'
...
**JAVA Script** : 
- 자바스크립트 예제문 풀이
- 배열 Array : 여러 개의 데이터를 담을 수 있는 특별한 변수로 인덱스 번호로 관리, 배열의 값은 변할 수 있음
- String 객체
- 함수 function
...
- return 키워드: 함수 안에서 선언된 변수나 값의 경우 외부에서 접근 불가능

# 240425 #
**JAVA Script** : 
- break 키워드 : 이 키워드를 만나는 즉시 구문을 벗어남. if, while, switch, for 문
- continue 키워드 : 이 키워드를 만나면 건너띄고 다시 반복문의 처음으로 돌아감 (반복문에서만 사용 됨)
- isNaN();
- 객체 Object : 객체, 함수, 배열, 정규 표현식 등 변형 가능한 데이터들의 집합
- window 객체는 생략 가능
...
- Date 객체 : 날씨와 시간을 생성하는 내장 객체이자 생성자 함수
- Math 객체 : 수학과 함수를 위한 속성(property)를 제공하는 내장객체
